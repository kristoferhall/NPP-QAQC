...
for (i in 1:length(raw.data.sheets)) {
  ...
  raw.data.file$field_sheet_name <- as.factor(list.files(location_raw.data.sheets, pattern=glob2rx("npp*csv"))[i])
  ...
  if (any(!colnames(raw.data.file) %in% legal.col.names)) {
    print(paste("A column name in file", raw.data.file$field_sheet_name[1], "needs to be corrected before continuing."))
    print(paste0("The following are not legal names: ", colnames(raw.data.file)[!colnames(raw.data.file) %in% legal.col.names], sep = ""))
    break
  }
...
colnames(all.quads) <- tolower(colnames(all.quads))
...
all.quads$site <- str_to_lower(all.quads$site)    # NEED TO REMEMBER TO CHANGE BACK A FEW SITE NAMES with mixed case later in program after checks are run (EDGE_black, etc. - see cheatsheet)
...
if("species" %in% colnames(raw.data)) {raw.data$species <- as.factor(toupper(raw.data$species))}
